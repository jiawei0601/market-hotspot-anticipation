...
INDUSTRY_ALLOWED = frozenset({
    ...
    "電子工業",
    "其他電子類",
    # ---- 0007-v3 資料驅動擴充（2026-07-04，見下方與 ADR 0007「修正紀錄三」）----
    "其他",
    "創新板股票",
    "化學工業",
    "塑膠工業",
...
第三筆 pre-registration 修正（2026-07-04，見 ADR 0007「修正紀錄三」）：允許清單語意擴充。
`data/sector_membership/` 板塊登記簿收錄了傳產（Traditional_Recovery：塑化/水泥/化學/
油電燃氣）與部分材料循環經濟股（Semiconductor_Materials_Recycling：官方產業分類落於
「其他」「創新板股票」「綠能環保類」）的成員，皆不在原「電子供應鏈十類」清單內，
導致這些已登記入簿的成員與 universe 篩選機制完全或大部分互斥。經使用者裁決採選項 B
（擴充允許清單，非僅維持登記簿脫鉤），依「枚舉法」逐一比對全部 13 個板塊登記簿成員的
`industry_category` 對照現行允許清單，列出「已入簿成員所屬、但不在允許清單」的完整類別
清單並全數納入：
  - 「其他」（6803崑鼎、8390金益鼎、9955佳龍，Semiconductor_Materials_Recycling）
  - 「創新板股票」（7610聯友金屬-創，Semiconductor_Materials_Recycling）
  - 「綠能環保類」（6894衛司特，Semiconductor_Materials_Recycling）
  - 「塑膠工業」（1301台塑、1303南亞、1312國喬、1326台化，Traditional_Recovery）
  - 「水泥工業」（1101台泥、1102亞泥、1103嘉泥、1104環泥、1108幸福，Traditional_Recovery）
  - 「油電燃氣業」（6505台塑化，Traditional_Recovery）
  - 「化學工業」（1714和桐，Traditional_Recovery）
誠實聲明：允許清單的語意自此輪起，從「電子供應鏈相關類別」擴為「登記簿板塊涵蓋之產業」，
不再是單純的電子供應鏈濾網；「其他」為官方分類系統的通用未分類桶（母體中 151 檔），
本次納入僅因登記簿證據要求，非代表已對「其他」桶內全部個股逐一審查適格性。
後續如新增板塊引入尚未涵蓋的產業分類，需依本輪方法同步枚舉、修訂並記錄，不可讓允許清單
與登記簿實際涵蓋範圍脫節。詳見 ADR 0007「Pre-registration 修正紀錄三」。

注意：此為「現行分類回溯套用到歷史月份」，見 ADR 0007 誠實聲明 (1)。"""

RULES_VERSION = "0007-v3"
"""快照內記錄的規則版本標籤，供未來規則變動時追溯是哪個版本產出的快照。
版本沿革：0007-v1＝首版（8 類＋電子工業）；0007-v2＝補上櫃「其他電子類」字尾變體
（2026-07-04 pre-registration 修正，見 ADR 0007 修正紀錄一/二）；0007-v3＝資料驅動擴充
允許清單以涵蓋登記簿板塊成員（其他/創新板股票/綠能環保類/塑膠工業/水泥工業/油電燃氣業/
化學工業，2026-07-04 pre-registration 修正三，見 ADR 0007 修正紀錄三）。
