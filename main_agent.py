...
def report_writer_node(state: MarketHotspotState) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        f"6. 提及任何個股，一律以『代號 中文名』並列呈現（例：3324 雙鴻），禁止只寫代號、只寫名稱或使用英文代稱。"
        + ("\n7. 本板塊無世代規格登記，全篇禁止出現 Vera_Rubin/Feynman/Feynman_Next 等 GPU 世代名稱或任何自行編造的世代代號。" if spec_missing else "")
    )
...
def quality_critic_node(state: MarketHotspotState) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        f"另須檢查【個股呈現格式】：抽查報告中提及的個股是否一律以『代號 中文名』並列呈現（例：3324 雙鴻）；"
        f"若通篇提及個股時只有代號沒有中文名、或只有名稱（含英文代稱）沒有代號，一律判 FAIL 並具體指出哪些個股呈現格式不合格。\n\n"
        f"【報告內容】：\n{report}\n"
    )
